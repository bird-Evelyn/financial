...
# 1. 设定时间范围
#end_date = datetime.today() - timedelta(days=1)  #TODO：始终设置时间为昨天
#start_date = end_date - timedelta(days=182)      #TODO：查看近半年的数据
...
def fetch_data_with_retry(ts_code, max_retries=3, retry_delay=5):
    ...
    while attempt < max_retries:
        try:
            df = pro.daily(ts_code=ts_code, start_date=start_str)
            if df.empty:
